...
async def change_index_audio(bot, msg):
    ...
    try:
        ...
        downloaded = await reply.download(progress=progress_message, progress_args=("🚀 Download Started... ⚡️", sts, c_time, new_name, msg.from_user.mention, "change index audio"))
    except Exception as e:
    ...
    if filesize > FILE_SIZE_LIMIT:
        ...
        try:
            await bot.send_document(
                ...
                caption=cap,
                progress=progress_message,
                progress_args=("💠 Upload Started... ⚡️", sts, c_time, new_name, msg.from_user.mention, "change index audio")
            )
        except Exception as e:
            return await sts.edit(f"Error: {e}")
...
async def change_index_subtitle(bot, msg):
    ...
    try:
        # Download the media file
        downloaded = await reply.download(progress=progress_message, progress_args=("🚀 Download Started... ⚡️", sts, c_time, media.file_name, msg.from_user.username, "changeindexsub"))
    except Exception as e:
        await safe_edit_message(sts, f"Error downloading media: {e}")
        return
    ...
    if filesize > FILE_SIZE_LIMIT:
        ...
        try:
            await bot.send_document(msg.chat.id, document=output_file, file_name=output_filename, thumb=file_thumb, caption=cap, progress=progress_message, progress_args=("💠 Upload Started... ⚡", sts, c_time, output_filename, msg.from_user.username, "changeindexsub"))
        except Exception as e:
            return await safe_edit_message(sts, f"Error: {e}")
