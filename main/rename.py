...
async def change_index_subtitle(bot, msg):
    ...
    user_id = msg.from_user.id
    username = msg.from_user.username or msg.from_user.first_name
    task_id = await db.add_task(user_id, username, "Change Subtitle Index", "Queued")
    await bot.send_message(GROUP, f"Change Subtitle Index Task is added by {username} ({user_id})")
    ...
    try:
        ...
        if filesize > FILE_SIZE_LIMIT:
            ...
            try:
                await bot.send_document(msg.chat.id, document=output_file, file_name=new_name, thumb=file_thumb, caption=cap, progress=progress_message, progress_args=("💠 Upload Started... ⚡", sts, c_time))
            except Exception as e:
                return await safe_edit_message(sts, f"Error: {e}")
...
async def change_index_audio(bot, msg):
    ...
    reply = msg.reply_to_message
    if not reply:
        return await msg.reply_text("Please reply to a media file with the index command\nFormat: `/changeindexaudio a-3 -n new_name.mkv` (Audio)")
    ...
    if not index_cmd or not index_cmd.startswith("a-"):
        return await msg.reply_text("Invalid format. Use `/changeindexaudio a-3 -n new_name.mkv` for audio.")
    ...
    task_id = await db.add_task(user_id, username, 'changeindexaudio', 'downloading', new_name)
    sts = await msg.reply_text(f"🚀 Task `{task_id}`: Downloading media... ⚡")
    ...
    await sts.edit(f"🚀 Task `{task_id}`: Changing audio indexing... ⚡")
    ...
    try:
        ...
        await db.update_task(task_id, f"failed: {e}")
        await sts.edit(f"❗ Error executing FFmpeg: {e}")
        return
    ...
    await db.update_task(task_id, 'uploading')
    await sts.edit(f"🚀 Task `{task_id}`: Uploading... ⚡")
    c_time = time.time()
...
async def start_merge_command(bot, msg: Message):
    ...
    # Add task to the database
    username = msg.from_user.username or msg.from_user.first_name
    task_id = await db.add_task(user_id, username, "Video Merge", "Queued")
    await bot.send_message(GROUP, f"Video Merge Task is added by {username} ({user_id})")
...
async def start_video_merge_command(bot, msg: Message):
    ...
    merge_state[user_id]["output_filename"] = new_name
...
async def merge_and_upload(bot, msg: Message):
    ...
    new_name = merge_state[user_id].get("output_filename", "merged_output.mp4")  # Default output filename
    output_path = f"{new_name}"

    # Update task status to "Started"
    await db.update_task(user_id, "Video Merge", "Started")
    ...
    try:
        ...
        await db.update_task(user_id, "Video Merge", "Completed")

    except Exception as e:
        await sts.edit(f"❌ Error: {e}")

        # Update task status to "Failed"
        await db.update_task(user_id, "Video Merge", "Failed")
