...
async def changemultitask(bot, msg: Message):
    ...
    if '-p' in msg.command:
        ...
        output_file = os.path.splitext(intermediate_file)[0] + "_photo" + os.path.splitext(intermediate_file)[1]

        await safe_edit_message(sts, "💠 Adding photo attachment... ⚡")
        try:
            # Function to add photo attachment (assume it's defined elsewhere)
            add_photo_attachment(intermediate_file, attachment_path, output_file)
        except Exception as e:
    ...
    await safe_edit_message(sts, "💠 Changing metadata... ⚡")
    try:
        change_video_metadata(output_file, video_title, audio_title, subtitle_title, output_filename)
    except Exception as e:
        await safe_edit_message(sts, f"Error changing metadata: {e}")
        os.remove(downloaded)
        os.remove(intermediate_file)
    ...
    if filesize > FILE_SIZE_LIMIT:
        ...
        button = [[InlineKeyboardButton("☁️ CloudUrl ☁️", url=f"{file_link}")]]
        await msg.reply_text(
            f"**File successfully changed audio index, attached photo, and updated metadata, then uploaded to Google Drive!**\n\n"
            f"**Google Drive Link**: [View File]({file_link})\n\n"
...
async def changemultitask1(bot, msg: Message):
    ...
    try:
        ...
        # Download the file
        downloaded = await reply.download(progress=progress_message, progress_args=("🚀 Download Started... ⚡️", sts, c_time, output_filename, username, "Change Index and Metadata"))
        ...
        if filesize > FILE_SIZE_LIMIT:
            ...
            try:
                await bot.send_document(msg.chat.id, document=output_file, file_name=output_filename, thumb=file_thumb, caption=cap, progress=progress_message, progress_args=("💠 Upload Started... ⚡", sts, c_time, output_filename, username, "Change Index and Metadata"))
            except Exception as e:
                return await safe_edit_message(sts, f"Error: {e}")
