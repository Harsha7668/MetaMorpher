...
async def changemultitasks(bot, msg: Message):
    ...
    await safe_edit_message(sts, f"💠 Changing audio indexing... ⚡\nTask ID: {task_id}")
    process = await asyncio.create_subprocess_exec(*ffmpeg_cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    stdout, stderr = await process.communicate()
    ...
    if '-p' in msg.command:
        ...
        await safe_edit_message(sts, f"💠 Adding photo attachment... ⚡\nTask ID: {task_id}")
        try:
            # Function to add photo attachment (assume it's defined elsewhere)
            add_photo_attachment(intermediate_file, attachment_path, output_file)
        except Exception as e:
    ...
    # Update metadata
    await safe_edit_message(sts, f"💠 Changing metadata... ⚡\nTask ID: {task_id}")
    try:
        change_video_metadata(output_file, video_title, audio_title, subtitle_title, output_filename)
    except Exception as e:
        await safe_edit_message(sts, f"Error changing metadata: {e}")
    ...
    await safe_edit_message(sts, f"💠 Uploading... ⚡\nTask ID: {task_id}")
    c_time = time.time()
